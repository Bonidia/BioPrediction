            ...
            for i in range(len(fasta)):
                for j in range(len(fasta[i])):
                    ...
                    if 1 in features_nucleot:
                        ...
                        subprocess.run(['python', './source/MathFeature/methods/ExtractionTechniques.py',
                    ...
                    if 2 in features_nucleot:
                        ...
                        subprocess.run(['python', './source/MathFeature/methods/ExtractionTechniques.py', '-i',
                    ...
                    if 4 in features_nucleot:
                        ...
                        subprocess.run(['python', './source/MathFeature/methods/Kgap.py', '-i',
                    ...
                    if 5 in features_nucleot:
                        ...
                        subprocess.run(['python', './source/MathFeature/methods/CodingClass.py', '-i',
                                        preprocessed_fasta, '-o', dataset, '-l', labels[i]],
                                        stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
                        datasets.append(dataset)
                    ...
                    if 6 in features_nucleot:
                        ...
                        subprocess.run(['python', './source/MathFeature/methods/FickettScore.py', '-i',
                                        preprocessed_fasta, '-o', dataset, '-l', labels[i],
                    ...
                    if 7 in features_nucleot:
                        ...
                        subprocess.run(['python', './source/MathFeature/methods/EntropyClass.py', '-i',
                                        preprocessed_fasta, '-o', dataset, '-l', labels[i],
                                        '-k', '5', '-e', 'Shannon'],
                                        stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
                        datasets.append(dataset)
                    ...
                    if 8 in features_nucleot:
                        ...
                        subprocess.run(['python', './source/MathFeature/methods/FourierClass.py', '-i',
                                        preprocessed_fasta, '-o', dataset, '-l', labels[i],
                    ...
                    if 9 in features_nucleot:
                        ...
                        subprocess.run(['python', './source/other-methods/FourierClass.py', '-i',
                                        preprocessed_fasta, '-o', dataset, '-l', labels[i],
                    ...
                    if 10 in features_nucleot:
                        ...
                        subprocess.run(['python', './source/other-methods/TsallisEntropy.py', '-i',
                                        preprocessed_fasta, '-o', dataset, '-l', labels[i],
                    ...
                    if '12' in features_nucleot:
                        ...
                        labels_list = [self.flabel] + [self.flabel] 
                        text_input = ''
                        for i in range(len(fasta_list)):
                            text_input += fasta_list[i] + '\n' + labels_list[i] + '\n'

                        subprocess.run(['python', './source/MathFeature/methods/MappingClass.py',
                ...
                if 13 in features_nucleot:        
                    ...
                    data_kmer = rev_kmer.make_revckmer_vec(open(fasta_list[0]))
                    data_kmer = pd.DataFrame(data_kmer)
                    data_kmer = data_kmer.add_prefix('Rev_kmer_')
                    dataset = os.path.join(self.path, 'Rev_kmer_'  + self.ftype + '.csv')
                    data_kmer.to_csv(dataset, index=False)
                    datasets.append(dataset)

                if 14 in features_nucleot: 
                    psednc = PseDNC()
                    data_psednc = psednc.make_psednc_vec(open(fasta_list[0]))
                    data_psednc = pd.DataFrame(data_psednc)
                    data_psednc = data_psednc.add_prefix('Pse_dnc_')
                    dataset = os.path.join(self.path, 'Pse_dnc_'  + self.ftype + '.csv')
                    data_psednc.to_csv(dataset, index=False)
                    datasets.append(dataset)
                
                if 15 in features_nucleot:
                    pseknc = PseKNC()
                    data_pseknc = pseknc.make_pseknc_vec(open(fasta_list[0]))
                    data_pseknc = pd.DataFrame(data_pseknc)
                    data_pseknc = data_pseknc.add_prefix('Pse_knc_')
                    dataset = os.path.join(self.path, 'Pse_knc_'  + self.ftype + '.csv')
                    data_pseknc.to_csv(dataset, index=False)
                    datasets.append(dataset)

                if 16 in features_nucleot:
                    sc_psednc = SCPseDNC()
                    data_sc_psednc = sc_psednc.make_scpsednc_vec(open(fasta_list[0]), all_property=True)
                    data_sc_psednc = pd.DataFrame(data_sc_psednc)
                    data_sc_psednc = data_sc_psednc.add_prefix('SCPseDNC_')
                    dataset = os.path.join(self.path, 'SCPseDNC_'  + self.ftype + '.csv')
                    data_sc_psednc.to_csv(dataset, index=False)
                    datasets.append(dataset)
                    
                if 17 in features_nucleot:
                    sc_psetnc = SCPseTNC(lamada=2, w=0.05)
                    data_sc_psetnc = sc_psetnc.make_scpsetnc_vec(open(fasta_list[0]), all_property=True)
                    data_sc_psetnc = pd.DataFrame(data_sc_psetnc)
                    data_sc_psetnc = data_sc_psetnc.add_prefix('SCPseTNC_')
                    dataset = os.path.join(self.path, 'SCPseTNC_'  + self.ftype + '.csv')
                    data_sc_psetnc.to_csv(dataset, index=False)
                    datasets.append(dataset)
                
                if 18 in features_nucleot:
                    dac = DAC(2)
                    data_dac = dac.make_dac_vec(open(fasta_list[0]), all_property=True)
                    data_dac = pd.DataFrame(data_dac)
                    data_dac = data_dac.add_prefix('DAC_')
                    dataset = os.path.join(self.path, 'DAC_'  + self.ftype + '.csv')
                    data_dac.to_csv(dataset, index=False)
                    datasets.append(dataset)
                
                if '19' in features_nucleot:
                    tac = TAC(2)
                    data_tac = tac.make_tac_vec(open(fasta_list[0]), all_property=True)
                    data_tac = pd.DataFrame(data_tac)
                    data_tac = data_tac.add_prefix('TAC_')
                    dataset = os.path.join(self.path, 'TAC_'  + self.ftype + '.csv')
                    data_tac.to_csv(dataset, index=False)
                    datasets.append(dataset)
                
                if '20' in features_nucleot:
                    tcc = TCC(2)
                    data_tcc = tcc.make_tcc_vec(open(fasta_list[0]), all_property=True)
                    data_tcc = pd.DataFrame(data_tcc)
                    data_tcc = data_tcc.add_prefix('TCC_')
                    dataset = os.path.join(self.path, 'TCC_'  + self.ftype + '.csv')
                    data_tcc.to_csv(dataset, index=False)
                    datasets.append(dataset)
                
                if '21' in features_nucleot:
                    tacc = TACC(2)
                    data_tacc = tacc.make_tacc_vec(open(fasta_list[0]), all_property=True)
                    data_tacc = pd.DataFrame(data_tacc)
                    data_tacc = data_tacc.add_prefix('TACC_')
                    dataset = os.path.join(self.path, 'TACC_'  + self.ftype + '.csv')
                    data_tacc.to_csv(dataset, index=False)
                    datasets.append(dataset)
            ...
            for i in range(len(fasta)):
                for j in range(len(fasta[i])):
                    ...
                    if 1 in features_amino:
                        ...
                        subprocess.run(['python', './source/MathFeature/methods/EntropyClass.py',
                                        '-i', preprocessed_fasta, '-o', dataset, '-l', labels[i],
                    ...
                    if 2 in features_amino:
                        ...
                        subprocess.run(['python', './source/other-methods/TsallisEntropy.py',
                                        '-i', preprocessed_fasta, '-o', dataset, '-l', labels[i],
                                        '-k', '5', '-q', '2.3'], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
                        datasets.append(dataset)
                    ...
                    if 5 in features_amino:
                        ...
                        subprocess.run(['python', './source/MathFeature/methods/ComplexNetworksClass-v2.py', '-i',
                                        preprocessed_fasta, '-o', dataset, '-l', labels[i],
                    ...
                    if 7 in features_amino:
                        ...
                        subprocess.run(['python', './source/MathFeature/methods/ExtractionTechniques-Protein.py', '-i',
                                        preprocessed_fasta, '-o', dataset, '-l', labels[i],
                    ...
                    if '10' in features_amino:
                        ...
                        dataset = os.path.join(self.path, 'Fourier_Integer_'  + self.flabel + '.csv')
                        labels_list = [self.flabel] + [self.flabel]
                        text_input = ''
                        for i in range(len(fasta_list)):
                            text_input += fasta_list[i] + '\n' + labels_list[i] + '\n'

                        subprocess.run(['python', './source/MathFeature/methods/Mappings-Protein.py',
                                        '-n', str(len(fasta_list)), '-o',
                    ...
                    if 11 in features_amino:
                        dataset = os.path.join(self.path, 'Fourier_EIIP_'  + self.flabel + '.csv')
                        labels_list = [self.flabel] + [self.flabel]
                        text_input = ''
                        for i in range(len(fasta_list)):
                            text_input += fasta_list[i] + '\n' + labels_list[i] + '\n'
                    ...
                    if '12' in features_amino:
                        dataset = os.path.join(self.path, 'EIIP_'  + self.flabel + '.csv')
                        labels_list = [self.flabel] + [self.flabel]
                        text_input = ''
                        for i in range(len(fasta_list)):
                            text_input += fasta_list[i] + '\n' + labels_list[i] + '\n'
                        ...
                        df = pd.read_csv(dataset, names=colnames, header=None)
                        f.rename(columns={df.columns[0]: 'nameseq', df.columns[-1]: 'label'}, inplace=True)
                        df = df.iloc[:, :-1]
                        df.to_csv(dataset, index=False)
                        datasets.append(dataset)
                        
                    if '13' in features_amino:
                        dataset = os.path.join(self.path, 'AAAF_'  + self.flabel + '.csv')
                        labels_list = [self.flabel] + [self.flabel]
                        text_input = ''
                        for i in range(len(fasta_list)):
                            text_input += fasta_list[i] + '\n' + labels_list[i] + '\n'
...
def make_dataset(data1, data2, label_1, label_2, table, output, arq_name):
    ...
    if len(table.columns)-1 != 0:
        ...
        data_names = data1['nameseq'] +'-'+ data2['nameseq']
        fnameseq = data_names.to_frame()
        
        data1 = data1.drop('nameseq', axis=1)
        data2 = data2.drop('nameseq', axis=1)
        data_concat = data1.join(data2)
        y = table
...
def objective_rf(space):
    ...
    descriptors_nucleot = {}
    nucleot_space =[0,4,16,64,320,10,2,5,19,19,5,0,0,44,19,65,54,88,76,24]
    add = 0
    ...
    descriptors_amino = {}
    amino_space =[0,5,5,5,5,78,400,20,400,0,0,19]
