...
def randomize_samples(targets, _class, n_samples=1):
    ...
    try:
        ...
        return map(int, class_samples[chosen,0])
    except ValueError:
        raise ValueError(
...
def generate_all_plots(model, train, test, preds, feature_names, path='explanations', n_samples=3):
    ...
    if len(classes) <= 2:
        raise ValueError(
            (f"{os.path.basename(__file__)} shouldn't be used to handle binary classification problems. To generate "
             "the interpretability report properly, please execute the binary classification script with the "
             "same configuration.")
        )
    ...
    generated_plt[SUMMARY] = []
    generated_plt[WATERFALL] = []
...
def build_interpretability_report(generated_plt,  n_samples, report_name="interpretability.pdf", directory="."):
    ...
    report.insert_figure_on_doc(generated_plt[SUMMARY])
    ...
    report.insert_figure_on_doc(generated_plt[WATERFALL])
